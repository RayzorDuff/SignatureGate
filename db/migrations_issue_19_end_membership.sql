...
SELECT m.member_id, m.status, m.membership_ended_at, m.membership_end_reason,
  (m.status = 'active'
    AND EXISTS (SELECT 1 FROM public.contributors c
      WHERE c.person_id=m.person_id AND c.contributor_type='individual'
        AND c.status='active')
...
BEGIN
  ...
  IF v_contributor_id IS NULL THEN
    RAISE EXCEPTION 'Enable this individual as a contributor before ending membership';
  END IF;
